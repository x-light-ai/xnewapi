...
func ConvertCodexResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	if typeStr == "response.created" {
		...
		output = appendSSEEventBytes(output, "content_block_stop", template, 2)
		...
		if itemType == "function_call" {
			...
			template, _ = sjson.SetBytes(template, "index", params.BlockIndex)
			template, _ = sjson.SetBytes(template, "content_block.id", shortenCodexCallIDIfNeeded(sanitizeClaudeToolID(itemResult.Get("call_id").String())))
			{
