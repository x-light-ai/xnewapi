// FORK-CUSTOM: Benchmark the CPA-derived Claude-to-Codex request translator.
package codex
...
func BenchmarkConvertClaudeRequestToCodexLargeHistory(b *testing.B) {
	for _, turns := range []int{16, 64} {
		b.Run(strconv.Itoa(turns)+"_turns", func(b *testing.B) {
			...
			require.True(b, gjson.ValidBytes(ConvertClaudeRequestToCodex("gpt-5.4", request, false)), "translator generated invalid Codex JSON")
			b.ReportAllocs()
